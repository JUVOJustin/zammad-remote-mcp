 ...
 *
 * Two consumers on the reading side: the plain previews a model decides an
 * ending by (`appended_text`, `zammad_get_group_signature.text`), and the
 * already-signed-as-text check in `appendGroupSignature`.
 *
 * One on the writing side, which is why this cannot be loosened for the previews
 * alone: `composeBody` (compose.ts) renders the body with it for the article
 * types Zammad transmits verbatim — SMS, Telegram, Facebook, WhatsApp — where
 * nothing downstream converts and this is the only pass there is. An email body
 * still goes out as HTML with Zammad generating the text part itself.
 *
